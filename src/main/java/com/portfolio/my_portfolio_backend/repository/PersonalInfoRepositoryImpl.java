...
public class PersonalInfoRepositoryImpl implements IPersonalInfoRepository {
    ...
    private final RowMapper<PersonalInfo> personalInfoRowMapper = (ResultSet rs, int rowNum) -> {
        ...
        info.setYearsOfExperience(rs.getObject("years_of_experience", Integer.class)); // usar getObject para manejar
                                                                                       // null
    ...
    // List<PersonalInfo> results = jdbcTemplate.query(sql, personalInfoRowMapper,
    // id);
    ...
    public Optional<PersonalInfo> findById(Long id) {
        String sql = "SELECT * FROM personal_info WHERE id = ?";
        try {
            PersonalInfo info = jdbcTemplate.queryForObject(sql, personalInfoRowMapper, id);
